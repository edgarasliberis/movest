...
Lorem ipsum dolor sit amet, consectetur adipiscing elit. Duis eu lorem eget sem mollis venenatis ut sed augue. Sed euismod erat dignissim aliquet tincidunt. Phasellus pulvinar, erat viverra gravida fringilla, nulla tellus mollis tortor, sit amet pretium lacus tellus non lacus. Donec ullamcorper velit eget elit aliquam, a placerat lectus blandit. Integer non justo quis neque dapibus dictum. Interdum et malesuada fames ac ante ipsum primis in faucibus. Curabitur ipsum lorem, dapibus non diam nec, molestie porttitor sem. Nam facilisis porttitor aliquet. 

\section{Detectability of embedding algorithms}

Most of the detectability evaluation was done in the previous chapter to justify further design improvements. This also allowed to evaluate the how effective implemented steganalysis techniques are. Remaining interesting aspects are covered in this section.
...
One of such decisions is choosing whether to embed into the $x$ or $y$ component based on the phase of the motion vector. Xu's algorithm uses the $x$ component if the phase is an acute angle, and the $y$ component otherwise, claiming it results in less disturbance to the motion. However, if, for example, a motion vector consists of a large $y$ component and a small $x$ component, incrementing the small $x$ component (as the selection policy suggests) would result in more disturbance to the motion, so the motivation behind such criteria is unclear.
...
As explained in section \ref{rev-tech-theory}, the reversion technique considers differences between respective motion vectors of the original and the re-encoded (transcoded) video. Evaluating the effectiveness of this technique requires training a linear SVM classifier. Since SVMs do not require large datasets to give accurate results \textbf{cite?}, I have obtained a dataset of 55 freely-available stock videos with various degrees of motion, such as slow cloud movements or chaotic city timelapses. 28 of those were chosen to become stego videos, so that the ratio between stego and non-stego videos is roughly 50:50 (balanced dataset).

For each sequential (non-randomised) embedding algorithm, the training and evaluation is done as follows:
\begin{itemize}
\item To obtain ``original'' videos, Movest encoder is used to embed random data into the 28 stego videos at their full capacity. The remaining 27 non-stego videos are also passed though the encoder, but no data is embedded (dummy pass).
...
Table \ref{tbl:rev-tech} presents the classification accuracy for each embedding algorithm.
...
MSteg & $100.00\% \pm 0.00\%$ \\ 
F3 & $100.00\% \pm 0.00\%$ \\ 
F4 & $96.09\% \pm 0.20\%$ \\
Xu's algorithm & $78.91\% \pm 0.78\%$ \\
MVSteg & $60.31\% \pm 0.66\%$ \\
\hline
\end{tabular}
}
\egroup
...
The results are surprisingly good. My interpretation of the trend in the data is that the classifier performs very well against algorithms that do not exclude small motion vector component values during the embedding (Hide \& Seek, MSteg, F3 and F4), and worse on those that operate predominantly on large ones. This would also explain why Xu's algorithm performs worse than MVSteg: as we saw earlier in section \ref{breaking-xu}, due to its component selection logic Xu's algorithm can sometimes embed into small component values.
...
\item Embedding has been done using the Hide \& Seek algorithm, which leaves a significant footprint by modifying every motion vector. The full embedding capacity of every video was used up to embed a random bytes.  
...
To be able to interpret the data, we need to define a theoretical model that would fit the experiment \textbf{cite?}. Let $n$ be the number of answers that participants gave and $X_i$ ($i \in [1, n]$) denote whether an answer was correct, \emph{i.e.} $X_i = 1$ if a participant correctly picked out a stego video in a trial and 0 otherwise. If the null hypothesis holds, we expect answers to be just as good as random guesses. Therefore $X_i$'s can be modelled as random variables with $\mathbb{P}(X_i = 1) = 1/3$, or, in other words, $X_i$'s follow Bernoulli distribution with $p = 1/3$. 

To consider the overall data, we compute the sum of all $X_i$: $Y = \sum^n_{i=1} X_i$. For sufficiently large $n$, $Y$ follows the Binomial distribution with parameters $(n, p)$ \textbf{cite}. By de Moivre--Laplace theorem, $Y$ converges to the normal distribution with mean $np$ and variance $np(1-p)$ \textbf{cite}. This allows us to put 95\% confidence bounds on the value of $Y$~\cite[p.~316--318]{ai2-notes}:
...
\end{tabular}
\egroup
\caption{Results of the distinguishability of video steganography experiment. Column one corresponds to the demo trial with an evidently corrupted video, so it's excluded from the analysis.}
\label{tbl:detect-exp-res}
\end{table}

The data contains 108 answers ($n = \text{\# of participants} \times \text{\# of trials}$), so, according to the formula above, I expect $Y$ to be within the range $36 \pm 5$. The data has 32 correct answers, which is within the required range, so we \emph{cannot} conclude that humans \emph{are} able to identify implemented video steganography.
