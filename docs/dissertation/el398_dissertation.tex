...
Perhaps the most important aspect to consider when evaluating a steganography system is detectability. Most of the detectability evaluation was done in the previous chapter to justify further algorithm design improvements, which also allowed to evaluate how effective implemented steganalysis techniques are. This section covers remaining detectability evaluation points.
...
The results are surprisingly good. The result against all algorithms is similar to the proportion of motion vectors they modify. My interpretation of the trend in the data is that classifiers perform very well against algorithms that do not exclude small motion vector component values during the embedding and perform worse on algorithms that operate predominantly on large values (Xu's algorithm, MVSteg).

\subsection{Varying embedding capacity}
\label{eval-var-emb-cap}

Many of the previous attacks, such as the $\chi^2$ attack, relied on payload bits being embedded close together, because their cumulative effect made it easier to expose the embedding. This weakness is be addressed by using algorithms that spread the payload uniformly across the cover video.

To evaluate how effective this approach is, I tested the $\chi^2$ attack and the reversion technique classifier for MSteg against the Outguess 1.0\footnote{Outguess 1.0 is the randomised version of MSteg.} algorithm at different embedding capacity settings. Table \ref{tbl:detect-outguess} presents findings obtained on a fresh dataset (not used during classifier training) of 10 videos; entries show how many videos were picked up as stego by either steganalysis method.
