...
The project uses the widely-used MPEG video files as covers. The MPEG format should be explored in more detail to see whether the use of motion vectors for data hiding is feasible.
...
An MPEG video stream consists of two types of frames:
\begin{itemize}
...
\begin{itemize}
\item P-frames --- use only the previous frame as the baseline for difference measurement,
\item B-frames --- use both the preceding and following frame for difference measurements. 
\end{itemize}
...
The Movest decoder can be configured to use a special algorithm \texttt{DumpMvs} which outputs motion vector components in a human-readable text format that can later be imported to Matlab as a 3D matrix (\texttt{loadmvs.m}) in which the element at $(i, j, k)$ refers to a macroblock at $i$-th row, $j$-th column of the $k$-th frame. 
...
This project implements the Least Significant Bit (LSB) steganography, so the suite includes a function (\texttt{lsbplane.m}) to project least significant bits out of every motion vector component value. If the embedding algorithm sets the LSBs of motion vectors to exactly payload bits, LSBs can be immediately subjected to visual analysis to reveal any patterns in the data. This is best illustrated by an example.
...
\vspace{-15pt}
\centering
\resizebox{5.5cm}{!}{\input{img/movest-reversion-sketch.tex}}
...
The results are surprisingly good, which means this technique is suitable for automated steganalysis. It appears that the accuracy result for each algorithm is similar to the proportion of motion vectors it modifies. The classifier shows over 90\% accuracy for the first four algorithms, which, unfortunately, means that their embedding is easily detectable when the full embedding capacity is used up. To mitigate this, one can either use Xu's algorithm or MVSteg, or embed less data, as discussed in the following section.
...
The table also includes an entry for 0\% setting (no data embedded) to verify that attacks do not produce false positives, \emph{i.e.} non-stego videos classified as stego. These data suggest that the reversion technique classifier starts being effective at 50--60\% embedding capacity and the $\chi^2$ attack is only effective from 90\%. Therefore the embedding is undetectable using implemented steganalysis techniques, as long as the payload consumes less than a half of entire video's embedding capacity.
